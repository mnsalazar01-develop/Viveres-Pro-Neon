...
try:
    ...
    res_c = ejecutar_consulta_neon("SELECT id_campana, id_super, nombre_campana, estado_campana, fecha_inicio, fecha_fin, disponible FROM public.campanas where disponible = true;") or []
...
col_s1, col_s2, col_s3 = st.columns([1.2, 1.9, 1.1])

with col_s1:
    ids_supers_activos = {s.get('id_super'): s.get('nombre_supermercado') for s in res_s if s.get('id_super')}
    id_super_contexto = st.selectbox("🏪 Supermercado:", options=ids_supers_activos, format_func=lambda x: mapa_supers_ram.get(x, f"Super #{x}"))
...
with col_s3:
    ...
    layout_dinamico = config_zoom.get(columnas_elegidas, config_zoom[10])
